...
sg.LOOK_AND_FEEL_TABLE["Fachero"] = {
                                "BACKGROUND": "#191970",
                                "TEXT": "#D9B382",
                                "INPUT": "#D9B382",
                                "TEXT_INPUT": "#191970",
...
letras = {
        "a": os.path.join(PATH_FICHAS, "A.png"),
...
casillas = {
        "palabra_x2": os.path.join(PATH_TABLERO, "verde.png"),
...
sfx = {
    "correcto": os.path.join(PATH_SFX, "421002__eponn__correct.wav"),
    "incorrecto": os.path.join(PATH_SFX, "243700__ertfelda__incorrect.wav")
        }

lista_musica = [
        os.path.join(PATH_MUSICA, "Sergey_Cheremisinov_-_05_-_The_Healing.mp3"),
...
def turno_computadora(pj: bool,
    ...
    for i in range(50):
        ...
        for _ in range(randint(2, len(copia_bolsa))):
...
def terminar_juego(window: sg.PySimpleGUI.Window, puntos_jugador: int, puntos_maquina: int, fm: list, estado_fichas: dict, dif: str):
    ...
    for i in range(0, 7):
        ...
        restantes_jugador += get_datos_letra(estado_fichas[f"ficha_jugador_{i}"]["letra"])["puntaje"] if estado_fichas[f"ficha_jugador_{i}"]["letra"] is not None else 0
